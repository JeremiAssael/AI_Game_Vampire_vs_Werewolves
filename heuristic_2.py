...
#def compute_score_state(s):
    ...
    if (player == "vampires"):
        ...
        for p in range(0,nb_possible_directions):
            index_possible_directions[p] = dir_index_map(possible_direction[p][0],possible_direction[p][1]) 
        ...
        for i in range(nb_groups_humans):
            ...
            # Compute the distances
            d_v_h = distance_between_groups(x_w,y_w,x_h,y_h)
            for i_v in range(nb_groups_vampires):
                if(distance_between_groups(vampires_list[i_v][0],vampires_list[i_v][1],x_h,y_h) < d_v_h):
                    consider_group_humans = False
                    break;
        ...
        for possible_direction in possible_directions:
            ...
            # If there are humans, and how many
            h_b = is_battle(f_x_v,f_y_v,humans_list, width,height)
            m_b = is_battle(f_x_v,f_y_v,vampires_list,width,height)
            if(h_b > 0):
